...
void ftrace_add(int, vaddr_t, vaddr_t);
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->pc + 4; s->dnpc = s->pc + imm; IFDEF(CONFIG_FTRACE, ftrace_add(1, s->dnpc, s->pc)));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->pc + 4; s->dnpc = src1 + imm; IFDEF(CONFIG_FTRACE, ftrace_add(rs1 != 1, s->dnpc, s->pc)));
  ...
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, R(rd) = BITS(SEXT(src1, 32) * (uint64_t)src2, 63, 32));
  ...
  INSTPAT("??????? ????? ????? 011 ????? 11100 11", csrrc  , I, R(rd) = SR(imm); SR(imm) &= ~src1);
  INSTPAT("??????? ????? ????? 101 ????? 11100 11", csrrwi , I, R(rd) = SR(imm); SR(imm) = rs1);
  INSTPAT("??????? ????? ????? 110 ????? 11100 11", csrrsi , I, R(rd) = SR(imm); SR(imm) |= rs1);
  INSTPAT("??????? ????? ????? 111 ????? 11100 11", csrrci , I, R(rd) = SR(imm); SR(imm) &= ~rs1);
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, s->dnpc = isa_raise_intr(11, s->pc));  // ecall in machine mode 
